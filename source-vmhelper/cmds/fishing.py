...
c.execute("CREATE TABLE IF NOT EXISTS vmhelper_fishing (user_id VARCHAR(255), exp FLOAT(255,1))")
...
class fishing(Cog_Extension):
    ...
    async def fish(self,ctx,feature:str):
        nologin = 0;  
        if feature == 'exp': # 查詢經驗
            sql = f"SELECT * FROM vmhelper_fishing"
            c.execute(sql)
            myresult = c.fetchall()
            for row in myresult:
                user_id = row[0]
            ...
            sql = "SELECT * FROM vmhelper_fishing"
            c.execute(sql)
            myresult = c.fetchall()
            for row in myresult:
                user_id = row[0]
            ...
            if(nologin != 0):
                ...
                sql = "INSERT INTO vmhelper_fishing (user_id, exp) VALUES (%s,%s)"
            ...
            for row in myresult:
                ...
                if str(ctx.message.author.id) == str(user_id): # 有註冊
                    ...
                    if fish_val >= 1.2: # 大白鯊
                        exp2 = exp + 1.5
                        sql = "UPDATE vmhelper_fishing SET exp = %s WHERE user_id = %s"
