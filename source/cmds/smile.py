...
class smile(Cog_Extension):
    ...
    async def about(self,ctx):
        ...
        embed.set_author(name="我的原始碼", url="https://github.com/minexo79/SmileGuy")
        embed.set_thumbnail(url="https://cdn.pixabay.com/photo/2013/04/01/09/07/wink-98461_960_720.png")
        embed.add_field(name="目前版本", value=jdata_bot['Version'], inline=False)
        embed.add_field(name="機器人作者", value="minexo79", inline=False)
        embed.add_field(name="指令幫助", value=jdata_bot['help'], inline=False)
        embed.set_footer(text=embedconfig.footer)
    ...
    async def info(self,ctx):
        ...
        embed.add_field(name="伺服器人數：", value=f"{server_user}", inline=False)
        embed.add_field(name="文字頻道：", value=f"{text_channel}", inline=False)
        embed.add_field(name="語音頻道：", value=f"{voice_channel}", inline=False)
        embed.set_footer(text=embedconfig.footer)
        await ctx.send(embed=embed)
