...
class weather(Cog_Extension):
    ...
    @commands.command() #天氣
    async def wea(self,ctx,state:str):
        city = state
        ...
        if code == '404': # 城市輸入錯誤
            ...
            # embed
            embed=discord.Embed(title=f"🌏天氣資訊",description="資料來源:openweathermap.org",url="https://openweathermap.org/",color=embedconfig.color)
